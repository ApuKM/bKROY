        ...
        <div className="flex flex-col sm:flex-row gap-4 shrink-0">
          ...
          <div className="relative">
            <div className="w-full sm:w-56">
              <Select
                ...
                onChange={(val) => {
                  if (val) {
                    onFilterChange({ sort: String(val) });
                  }
                }}
                placeholder="Sort by"
              >
                ...
                <Select.Trigger className="bg-[#27272a] border-none rounded-xl text-sm text-white w-full flex justify-between items-center px-3 py-2.5">
                  {/* Left side: Icon + Selected Value */}
                  <div className="flex items-center gap-2.5">
                    <FiFilter className="text-zinc-400" size={16} />
                    <Select.Value />
                  </div>
                ...
                <Select.Popover className="bg-zinc-800 rounded-xl shadow-lg mt-1 border border-zinc-700">
                  <ListBox className="p-1 outline-none">
                    {sortOptions.map((option) => (
                      <ListBox.Item
                        ...
                        className="p-2 hover:bg-[#3f3f46] rounded-lg cursor-pointer text-sm text-white transition-colors outline-none"
                      >
                        <Label className="cursor-pointer text-zinc-300">
                          {option.label}
                        </Label>
          ...
          {hasActiveFilters && (
            <button
              ...
              className="flex items-center justify-center gap-2 bg-red-500/10 hover:bg-red-500/20 text-red-400 border border-red-500/20 px-4 py-3.5 rounded-xl transition-colors text-sm font-medium whitespace-nowrap"
            >
              <FiX size={16} />
              Clear
            </button>
